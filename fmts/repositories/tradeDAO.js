...
exports.getPrices = (category) => {
  ...
  if (category !== undefined) {
    prices = prices.filter((price) => price.instrument.categoryId === category);
  }
...
exports.performTrade = (order) => {
  ...
  if (order.direction == "S" && price != undefined) {
    // Sell
    if (
  ...
  } else if (order.direction == "B" && price != undefined) {
    // Buy
    if (
...
let generateTradeId = () => {
  let TID =
    generateAlphaNum() + "-" + generateAlphaNum() + "-" + generateAlphaNum();
  return TID;
};
